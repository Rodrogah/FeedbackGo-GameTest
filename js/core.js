...
// =========================================================
// CELEBRAÇÃO DE LEVEL UP (GAMIFICAÇÃO)
// Detecta subida de nível do usuário (via listeners do Firestore)
// e mostra um modal animado com confetti UMA única vez por nível.
// Guarda o último nível visto em localStorage por usuário.
// =========================================================
window.FG_LEVEL_CELEBRATION = {
    storageKey: function () {
        const uid = (window.currentUser && window.currentUser.id) ? window.currentUser.id : 'anon';
        return 'fg_seen_level_' + uid;
    },

    ultimoVisto: function () {
        const v = parseInt(localStorage.getItem(this.storageKey()) || '', 10);
        return Number.isFinite(v) ? v : null;
    },

    registrarNivelVisto: function (nivel) {
        try { localStorage.setItem(this.storageKey(), String(nivel)); } catch (e) { /* ignore */ }
    },

    // Chamado toda vez que o painel do usuário é atualizado com dados do Firestore.
    checkAndCelebrate: function (dadosUsuario, regrasEmpresa) {
        if (!dadosUsuario) return false;
        const novoLevel = parseInt(dadosUsuario.level || dadosUsuario.nivel || 1, 10) || 1;
        const visto = this.ultimoVisto();
        if (visto === null) { this.registrarNivelVisto(novoLevel); return false; }
        if (novoLevel <= visto || window.__fgCelebracoesAberto) return false;

        let moedasEstimadas = 0;
        if (regrasEmpresa && regrasEmpresa.coinsNivel) {
            const porNivel = parseInt(regrasEmpresa.coinsNivel, 10);
            if (porNivel > 0) moedasEstimadas = (novoLevel - visto) * porNivel;
        }

        let patenteNova = null;
        if (typeof window.getPatente === 'function' && typeof window.getProximaPatente === 'function') {
            const pAtual = window.getPatente(novoLevel);
            const pAnterior = window.getPatente(visto);
            if (pAtual && pAnterior && pAtual.nome !== pAnterior.nome) patenteNova = pAtual;
        }

        this.registrarNivelVisto(novoLevel);
        this.mostrarCelebracao(novoLevel, moedasEstimadas, patenteNova);
        return true;
    },

    mostrarCelebracao: function (novoLevel, moedas, patenteNova) {
        if (window.__fgCelebracoesAberto) return;
        window.__fgCelebracoesAberto = true;

        const corPrimaria = patenteNova ? patenteNova.cor : '#fbbf24';
        const gradiente = patenteNova ? patenteNova.gradient : 'linear-gradient(135deg, #f59e0b, #fbbf24)';
        const iconPatente = patenteNova ? (patenteNova.imagem
            ? `<img src="${encodeURI(patenteNova.imagem)}" onerror="this.outerHTML='<i class=&quot;${patenteNova.icone}&quot; style=&quot;color:${corPrimaria};font-size:40px;&quot;></i>'" style="width: 64px; height: 64px; object-fit: contain; filter: drop-shadow(0 6px 14px rgba(0,0,0,0.4));">`
            : `<i class="${patenteNova.icone}" style="color: ${corPrimaria}; font-size: 40px;"></i>`)
            : '<i class="fa-solid fa-arrow-trend-up" style="color: #fbbf24; font-size: 38px;"></i>';

        const overlay = document.createElement('div');
        overlay.id = 'levelUpCelebrationOverlay';
        overlay.setAttribute('role', 'dialog');
        overlay.setAttribute('aria-modal', 'true');
        overlay.setAttribute('aria-labelledby', 'levelUpTitle');
        overlay.style.cssText = 'position:fixed;inset:0;z-index:100001;display:flex;align-items:center;justify-content:center;background:rgba(15,23,42,0.72);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);animation:fgFadeIn 0.25s ease-out;padding:20px;';

        const card = document.createElement('div');
        card.style.cssText = `width:100%;max-width:420px;background:#ffffff;border-radius:24px;overflow:hidden;box-shadow:0 30px 80px rgba(0,0,0,0.45);animation:fgPopIn 0.45s cubic-bezier(0.34,1.56,0.64,1);text-align:center;color:#0f172a;`;

        const proxima = (typeof window.getProximaPatente === 'function' && patenteNova) ? window.getProximaPatente(novoLevel) : null;

        card.innerHTML = `
            <div style="padding:32px 24px 18px;background:${gradiente};color:#fff;">
                <div style="margin-bottom:10px;">${iconPatente}</div>
                <div id="levelUpTitle" style="font-size:12px;font-weight:800;letter-spacing:3px;text-transform:uppercase;opacity:0.85;">Parabéns!</div>
                <div style="font-size:34px;font-weight:950;line-height:1.1;margin-top:2px;text-shadow:0 2px 10px rgba(0,0,0,0.25);">NÍVEL ${novoLevel}</div>
                ${patenteNova ? `<div style="margin-top:10px;display:inline-block;background:rgba(255,255,255,0.22);border:1px solid rgba(255,255,255,0.35);border-radius:999px;padding:6px 16px;font-weight:900;font-size:14px;letter-spacing:0.5px;">NOVA PATENTE: ${patenteNova.nome.toUpperCase()}</div>`
                : `<div style="margin-top:10px;font-size:13px;font-weight:700;opacity:0.9;">Você subiu de nível!</div>`}
            </div>
            <div style="padding:22px 24px 26px;">
                ${moedas > 0 ? `
                <div style="display:flex;align-items:center;justify-content:center;gap:10px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:14px;padding:14px;margin-bottom:14px;">
                    <img src="Patentes/Moedas/Amount_GoCoins.svg" style="width:30px;height:30px;object-fit:contain;" onerror="this.outerHTML='<i class=&quot;fa-solid fa-coins&quot; style=&quot;color:#10b981;font-size:22px;&quot;></i>'">
                    <span style="font-weight:950;font-size:20px;color:#047857;">+${moedas} GoCoins</span>
                    <span style="font-size:12px;color:#059669;font-weight:700;">prêmio de nível</span>
                </div>` : ''}
                ${proxima ? `
                <div style="font-size:12px;font-weight:700;color:#64748b;">Próximo rank: ${proxima.nome} (Nível ${proxima.minLevel})</div>
                ` : '<div style="font-size:12px;font-weight:700;color:#64748b;">Rank máximo alcançado!</div>'}
                <button type="button" id="btnFecharLevelUp" style="margin-top:20px;width:100%;padding:14px;border:none;border-radius:14px;background:${gradiente};color:#fff;font-weight:900;font-size:15px;cursor:pointer;box-shadow:0 10px 24px -6px ${corPrimaria}aa;">CONTINUAR</button>
            </div>`;

        const close = () => {
            if (!overlay.isConnected) return;
            overlay.remove();
            window.__fgCelebracoesAberto = false;
            document.body.style.overflow = '';
        ...
        const btnClose = document.getElementById('btnFecharLevelUp');
        if (btnClose) {
            btnClose.addEventListener('click', close);
            setTimeout(() => btnClose.focus(), 80);
        }
        overlay.addEventListener('click', (e) => { if (e.target === overlay) close(); });
        document.addEventListener('keydown', onKey);

        const style = document.createElement('style');
        style.id = 'fgCelebrationStyles';
        style.textContent = '@keyframes fgFadeIn{from{opacity:0}to{opacity:1}}@keyframes fgPopIn{0%{transform:scale(0.6);opacity:0}100%{transform:scale(1);opacity:1}}';
        document.head.appendChild(style);

        this._soltarConfetti(corPrimaria);
    },

    _soltarConfetti: function (corPrincipal) {
        if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
        const canvas = document.createElement('canvas');
        canvas.style.cssText = 'position:fixed;inset:0;width:100%;height:100%;pointer-events:none;z-index:100002;';
        document.body.appendChild(canvas);
        const ctx = canvas.getContext('2d');
        const W = canvas.width = window.innerWidth;
        const H = canvas.height = window.innerHeight;
        const cores = [corPrincipal, '#fbbf24', '#10b981', '#38bdf8', '#a78bfa', '#f472b6', '#f97316', '#ffffff'];
        const partes = [];
        for (let i = 0; i < 160; i++) {
            partes.push({
        ...
        function frame(now) {
            ...
            partes.forEach(pc => {
                ...
                ctx.fillRect(-pc.tam / 2, -pc.tam / 2, pc.tam, pc.tam * 0.65);
                ctx.restore();
            });
...
// =========================================================
// TRAVA DE SCROLL DO FUNDO ENQUANTO HÁ JANELA ABERTA (MOBILE)
// Observa .modal-overlay (padrão hidden) e .mobile-modal-overlay (padrão active).
// =========================================================
(function () {
  function sincronizarScrollAberto() {
    if (!document.body) return;
    var abertos = false;
    var listraPadrao = document.querySelectorAll('.modal-overlay');
    for (var i = 0; i < listraPadrao.length; i++) {
      if (!listraPadrao[i].classList.contains('hidden')) { abertos = true; break; }
    }
    if (!abertos) {
      abertos = document.querySelectorAll('.mobile-modal-overlay.active').length > 0;
    }
    document.body.classList.toggle('mobile-modal-open', abertos);
  }
