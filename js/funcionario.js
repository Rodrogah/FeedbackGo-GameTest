...
async function showEmployeeSection(sec) {
  ...
  .then((querySnapshot) => {
      ...
      lista.forEach(t => {
          ...
          let ribbonColor = 'var(--color-success)'; 
          let badge = `<span class="badge" style="background: rgba(16, 185, 129, 0.1); color: #047857; border: 1px solid rgba(16, 185, 129, 0.2); font-weight: 700; border-radius: 8px; padding: 4px 10px; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px;">Concluída & Aprovada</span>`;
          
          if (pendente) {
              ribbonColor = t.feedbackAdmin ? 'var(--color-danger)' : 'var(--color-warning)';
              badge = t.feedbackAdmin 
                  ? `<span class="badge" style="background: rgba(239, 68, 68, 0.1); color: #b91c1c; border: 1px solid rgba(239, 68, 68, 0.2); font-weight: 700; border-radius: 8px; padding: 4px 10px; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px;">Devolvida c/ Erro</span>` 
                  : `<span class="badge" style="background: rgba(245, 158, 11, 0.1); color: #b45309; border: 1px solid rgba(245, 158, 11, 0.2); font-weight: 700; border-radius: 8px; padding: 4px 10px; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px;">Pendente</span>`;
          } else if (emRevisao) {
              ribbonColor = 'var(--color-info)';
              badge = `<span class="badge" style="background: rgba(59, 130, 246, 0.1); color: #1d4ed8; border: 1px solid rgba(59, 130, 246, 0.2); font-weight: 700; border-radius: 8px; padding: 4px 10px; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px;">Em Revisão</span>`;
          }
          ...
          html += `
          <div class="card" style="padding: 20px; border-radius: 16px; border: 1px solid var(--color-border); background: var(--color-bg-secondary); box-shadow: 0 4px 20px rgba(0,0,0,0.03); display: flex; flex-direction: column; gap: 0; position: relative; overflow: hidden; margin-bottom: 0;">
              <div style="position: absolute; left: 0; top: 0; bottom: 0; width: 5px; background: ${ribbonColor};"></div>
              <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 15px; padding-left: 12px;">
                  <div style="flex: 1; min-width: 250px;">
                      <div style="display: flex; align-items: center; gap: 12px; font-size: 12px; color: var(--color-text-secondary); margin-bottom: 8px;">
                          <span style="display: flex; align-items: center; gap: 5px;"><i class="fa-solid fa-user-tie" style="color: var(--color-primary);"></i> <strong style="color: var(--color-text-primary);">De: ${nomeAdmin}</strong></span>
                          <span>•</span>
                          <span style="display: flex; align-items: center; gap: 5px;"><i class="fa-solid fa-calendar-day"></i> Envio: ${dataFormatada}</span>
                      </div>
                      <h4 style="margin: 0 0 8px 0; font-size: 16px; font-weight: 700; color: var(--color-text-primary);">${t.title}</h4>
                      ${descricaoResumida ? `<p style="margin: 0 0 12px 0; font-size: 13px; color: var(--color-text-secondary); line-height: 1.5; white-space: pre-line;">${descricaoResumida}</p>` : ''}
                      
                      <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                          ${badge}
                          ${t.quantidade ? `<span class="badge" style="background: rgba(59, 130, 246, 0.08); color: #2563eb; border: 1px solid rgba(59, 130, 246, 0.15); font-weight: 700; border-radius: 8px; padding: 4px 10px; font-size: 11px;"><i class="fa-solid fa-chart-simple"></i> Vol: ${t.quantidade}</span>` : ''}
                      </div>
                      
                      ${t.feedbackAdmin ? `
                      <div style="margin-top: 12px; padding: 12px; background: rgba(239, 68, 68, 0.05); border: 1px dashed rgba(239, 68, 68, 0.25); border-radius: 10px; font-size: 13px; color: #dc2626; display: flex; align-items: flex-start; gap: 8px;">
                          <i class="fa-solid fa-triangle-exclamation" style="margin-top: 2px; flex-shrink: 0;"></i>
                          <div>
                              <strong>Ajuste Necessário:</strong>
                              <div style="opacity: 0.9; margin-top: 3px; white-space: pre-wrap; color: #dc2626 !important;">${t.feedbackAdmin}</div>
                          </div>
                      </div>` : ''}
                  </div>
                  <div style="display: flex; align-items: center; align-self: center; flex-shrink: 0;">
                      ${pendente
                          ? `<button class="btn btn-primary btn-small" onclick="abrirModalTarefa('${t.id}')" style="border-radius: 10px; font-weight: 700; display: flex; align-items: center; gap: 6px; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.2); white-space: nowrap;"><i class="fa-solid fa-reply"></i> ${t.feedbackAdmin ? 'Ajustar e Reenviar' : 'Responder'}</button>`
                          : emRevisao 
                              ? `<button class="btn btn-small" onclick="abrirModalTarefa('${t.id}')" style="border-radius: 10px; font-weight: 700; display: flex; align-items: center; gap: 6px; background: #3b82f6; color: white; border: none; box-shadow: 0 4px 12px rgba(59, 130, 246, 0.2); white-space: nowrap;"><i class="fa-solid fa-pen"></i> Editar Entrega</button>`
                              : `<button class="btn btn-small" disabled style="border-radius: 10px; font-weight: 700; display: flex; align-items: center; gap: 6px; background: rgba(148, 163, 184, 0.08); color: #64748b; border: 1px solid rgba(148, 163, 184, 0.2); cursor: default; white-space: nowrap;"><i class="fa-solid fa-circle-check" style="color: #10b981;"></i> Aprovada</button>`
                      }
                  </div>
              </div>
          </div>`;
            ...
            .onSnapshot(doc => {
                if (doc.exists) {
                    ...
                    if (typeof window.FG_LEVEL_CELEBRATION !== 'undefined' && window.FG_LEVEL_CELEBRATION) {
                        const cAtual = companies.find(x => String(x.id) === String(currentUser.companyId));
                        window.FG_LEVEL_CELEBRATION.checkAndCelebrate(u, cAtual ? cAtual.gamificacao : null);
                    }
...
window.solicitarResgateInternoComValor = async function(premioId, nomePremio, temCodigo = true) {
    ...
    solicitarResgateInterno(premioId, nomeCompleto, precoCoins, temCodigo);
};
